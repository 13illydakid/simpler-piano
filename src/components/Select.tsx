    ...
    >
      <input
        ...
        className={clsx(
          ...
          'hover:border hover:border-black focus:border-black focus:outline-none',
        )}
        onClick={(e) => {
          e.stopPropagation();
          toggleMenu();
      ...
      {loading && (
        <div className="absolute top-0 w-full h-full grid place-items-center pointer-events-none">
          <Loader size={24} height={24} className="animate-spin text-black" />
        </div>
      )}
      <div className="relative">
        <div className="absolute top-0 w-full z-10">
          <div
            ref={menuRef}
            className={clsx(
              'w-full h-full bg-white transition max-h-[250px] overflow-y-auto border border-black rounded-md',
              openMenu ? '' : 'hidden',
            )}
          >
            {options.map((option) => {
              return (
                <div
                  key={option}
                  className={clsx('p-1 cursor-pointer hover:bg-black')}
                  onClick={(e) => {
                    e.stopPropagation();
                    handleSelect(option);
                  }}
