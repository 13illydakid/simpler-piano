...
export function Phrases() {
  ...
  return (
    <>
      <Head>
        <title>Simpler Piano: Phrases</title>
      </Head>
      ...
      <div className={clsx('flex flex-col h-screen', 'h-[100dvh]')}>
        ...
        <div className="relative px-8 py-4 flex w-full justify-center gap-4 bg-gray-200">
          <div className="flex items-center gap-2 bg-white rounded-md p-2 text-black whitespace-nowrap">
            ...
            <ButtonWithTooltip tooltip="Increase BPM" onClick={() => player.increaseBpm()}>
              <ChevronUp className="text-black hover:text-red-600" />
            </ButtonWithTooltip>
            <ButtonWithTooltip tooltip="Decrease BPM" onClick={() => player.decreaseBpm()}>
              <ChevronDown className="text-black hover:text-red-600" />
            </ButtonWithTooltip>
          </div>
        ...
        <div className="flex basis-0 items-center justify-center gap-4 text-white">
          <PhrasesBtn
            ...
            className="bg-white text-red-600 border border-red-600 hover:bg-green-300"
          >
            Replay
          </PhrasesBtn>
          <PhrasesBtn onClick={handleNext} className="bg-red-600 hover:bg-black">
            Next
          </PhrasesBtn>
        </div>
      </div>
    ...
    <div className="flex flex-col gap-2 min-w-[150px]">
      ...
      <div className="relative w-full h-[40px]">
        ...
        <div
          ...
          className="absolute rounded-r-3xl h-[34px] bottom-0 bg-gradient-to-r from-white to-red-600"
          ref={progressRef}
        />
      </div>
    </div>
...
function StatDisplay(props: PropsWithChildren<{ label: string }>) {
  return (
    <div className="flex flex-col gap-2 min-w-[150px]">
      ...
      <span className="text-red-600 text-4xl font-semibold">{props.children}</span>
    </div>
  )
}
