    ...
    >
      ...
      >
        <span className="text-gray-300">
          Time: <span className="text-sm text-black" ref={timeSpanRef} />
        </span>
        <span className="text-gray-300">
          Measure: <span className="text-sm text-black" ref={measureSpanRef} />
        </span>
      </div>
      ...
      >
        ...
        <div
          ref={divRef}
          className={`absolute h-full w-full pointer-events-none bg-blue-600`}
          style={{ left: -width }}
        />
      </div>
      ...
      {rangeSelection && (
        <div ref={rangeRef} className="absolute h-full flex items-center pointer-events-none">
          <div className="absolute w-[calc(100%-10px)] h-4 bg-blue-700/40" />
          <div
            className="pointer-events-auto absolute left-0 cursor-pointer h-6 w-6 bg-blue-700/90 hover:bg-black/90 rounded-full -translate-x-1/2 transition"
            onPointerEnter={() => setPointerOver(true)}
            onPointerLeave={() => setPointerOver(false)}
            onPointerDown={() => (isDraggingL.current = true)}
          />
          <div
            className="pointer-events-auto absolute right-0 translate-x-1/2 cursor-pointer h-6 w-6 bg-blue-700/90 hover:bg-black/90 rounded-full transition"
